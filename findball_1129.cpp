    ...
    for (int i=1;i<=Height;i++)//颜色分割，bfs数组初始化
        for (int j=1;j<=Width;j++) {
            ...
            if (GoalColor==COLOR_Orange) {
                ...
                if (Color_of_pixel[i][j]==GoalColor) Color_of_pixel[i][j]=COLOR_Orange;//交换识别色为橙色
                else if (Color_of_pixel[i][j]==COLOR_Orange) Color_of_pixel[i][j]=GoalColor;
            }
        }
}
...
public:
  ...
  void image_change(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    image = *msg;
    /// opencv Mat format imgMat
    ROS_INFO("Image change");
    cv::Mat imgMat;
    imgMat = cv::Mat(image.height, image.width, CV_8UC3, cv::Scalar::all(0));
    ...
    // Canny parameter sensitive
//    cv::Canny(imggray, imggray, 200, 300);
    // Reduce the noise so we avoid false circle detection
//    GaussianBlur( src_gray, src_gray, Size(9, 9), 2, 2 );

    // Hough 圆变换
    cv::HoughCircles(imggray, circles, CV_HOUGH_GRADIENT, 1, imgMat.rows/16, 100, 100, 0, 0 );
